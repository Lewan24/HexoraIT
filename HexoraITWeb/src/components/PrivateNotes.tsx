...
function NotesEditor({ orgId }: { orgId: string }) {
  ...
  const run = async (action: () => Promise<void>) => {
    ...
    setError('')
    try { await action() }
    catch (err) { setError(err instanceof Error ? err.message : tr("Failed to save changes")) }
  ...
  return <div className="space-y-5 p-6 text-ink-primary">
    <div className="flex items-center justify-between gap-3">
      <div>
        <h1 className="text-xl font-semibold">{tr("Private notes")}</h1>
        <p className="text-sm text-ink-muted">{tr("Only you can access these notes in this organization.")}</p>
      </div>
      <button className={buttonClass} disabled={busy || loading || !!draft}
        onClick={() => setDraft({ id: '', title: '', content: '', updatedAt: '' })}>{tr("New note")}</button>
    </div>
    {error && <p role="alert" className="text-red-400">{error}</p>}
    {loading && <p>{tr("Loading notes…")}</p>}
    {!loading && !notes.length && !draft && <p className="text-ink-muted">{tr("No private notes yet.")}</p>}
    {draft && <fieldset disabled={busy} className="space-y-3">
      <label className="block">{tr("Title")} <input className={fieldClass} maxLength={200} value={draft.title}
          onChange={e => setDraft({ ...draft, title: e.target.value })} />
      </label>
      <label className="block">{tr("Note")} <textarea className={`${fieldClass} min-h-64`} maxLength={100000} value={draft.content}
          onChange={e => setDraft({ ...draft, content: e.target.value })} />
      </label>
      <div className="flex gap-2">
        <button className={buttonClass} disabled={!draft.title.trim()} onClick={save}>{busy ? tr("Saving…") : tr("Save")}</button>
    ...
    {notes.map(note => <article key={note.id} className="space-y-3 rounded-xl border border-edge-default p-4">
      ...
      <p className="text-xs text-ink-muted">{new Date(note.updatedAt).toLocaleString(locale())}</p>
      <div className="flex gap-2">
        <button className={buttonClass} disabled={busy || !!draft} onClick={() => setDraft({ ...note })}>{tr("Edit")}</button>
        <button className={buttonClass} disabled={busy || !!draft} onClick={() => {
          if (window.confirm(tr("Delete this private note?"))) void run(async () => {
